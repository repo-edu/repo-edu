...
test("Codex rebuttal excludes all pre-invocation usage and retains the new selection", async (t) => {
  ...
  await f.configure({
    stream: await phaseStream(
      "codex",
      'PHASE RESULT: {"status":"finished","file":"/REBUT.md","reason":null,"tier":null,"due":null,"clean":null,"accepted":null}',
    ),
...
test("the brief carries its pinned model and effort into the Codex invocation", async (t) => {
  ...
  await f.configure({
    stream: await phaseStream(
      "codex",
      'Brief written\nPHASE RESULT: {"status":"finished","file":"/peer plan/BRIEF.md","reason":null,"tier":null,"due":null,"clean":null,"accepted":null}',
    ),
    usage: {
