...
function runFile(command: string, args: readonly string[]): void {
...
function currentReleasePlatform():
  ...
  throw new Error(
    `Unsupported release preflight platform: ${process.platform}/${process.arch}`,
  )
}
...
const preflightPlatform = currentReleasePlatform()
runFile("pnpm", [
  "--filter",
  "@repo-edu/release",
  "license-gate",
