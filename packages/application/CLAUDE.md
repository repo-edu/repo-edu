...
- Analysis workflows are in `src/analysis-workflows/`: `analysis-handler.ts` (`analysis.run`), `blame-handler.ts` (`analysis.blame`), `discover-repos-handler.ts` (`analysis.discoverRepos`), plus `log-parser.ts`, `blame-parser.ts`, `snapshot-engine.ts`, `filter-utils.ts`, `repo-root.ts`, `ports.ts` (`AnalysisWorkflowPorts`). `repo-root.ts` validates the repository locator union: course-relative paths require clone-target source data, while absolute paths run without course data. Handlers use `GitCommandPort` only — there is no application-level analysis cache (a previous LRU/persistent cache was removed deliberately; see `analysis-workflows/CLAUDE.md`).
