...
Provides a matrix of `PersistedCourse` + `PersistedAppSettings` + CSV/JSON artifacts, keyed by tier (`small`/`medium`/`stress`) and preset (`shared-teams`/`task-groups`/`repobee-teams`/`no-backing`). The `repobee-teams` preset emits `backing: "repobee"` (no LMS connection), `no-backing` emits `backing: null`, and the LMS-backed presets emit `backing: "lms"`.
