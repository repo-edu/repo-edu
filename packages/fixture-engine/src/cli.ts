...
const TOP_OVERVIEW_LINES = [
  ...
  "            child with an LLM judge → _evaluate-<model>.md report.",
  "",
  "Options:",
  ...
  "      m23-r31-o2/                   # git repo from `fixture repo`",
...
function subcommandHelpBody(sub: Subcommand): string[] {
  ...
  if (sub === "init") {
    return [
      ...
      "",
      "With --from=<project.md>, also seeds a curated project by archiving it to",
  ...
  if (sub === "evaluate") {
    return [
      ...
      "Writes a Markdown report at <root>/_evaluate-<model>.md",
      ...
      ...opt(
        ...
        "<root>/_evaluate-<model>.md destination",
