...
import { workflowCatalog, type WorkflowId } from "@repo-edu/application-contract";
...
async function collectAppWorkflowIds(): Promise<string[]> {
...
describe("docs workflow alignment", () => {
  ...
  it("covers every workflow id invoked by packages/app runtime code", async () => {
    ...
    for (const workflowId of appWorkflowIds) {
      ...
      const metadata = workflowCatalog[workflowId as WorkflowId];
      assert.equal(
