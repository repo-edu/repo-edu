...
const appCoreBindingsDir = resolve(repoRoot, "packages/app-core/src/bindings")
const tsCommandsPath = resolve(appCoreBindingsDir, "commands.ts")
const tsTauriPath = resolve(bindingsDir, "tauri.ts")

const interfaceDir = resolve(repoRoot, "packages/backend-interface/src")
const interfacePath = resolve(interfaceDir, "index.ts")
const interfaceTypesPath = resolve(interfaceDir, "types.ts")
...
function generateInterface(manifest: CommandManifestStub): {
  content: string
...
function generateTauriBackend(manifest: CommandManifestStub): {
  content: string
  usedTypes: Set<string>
} {
  const usedTypes = new Set<string>()
...
async function generateAll(): Promise<void> {
  ...
  const { content: tauriContent } = generateTauriBackend(manifest)
  writeIfChanged(tsTauriPath, tauriContent)

  const { content: interfaceContent } = generateInterface(manifest)
  writeIfChanged(interfacePath, interfaceContent)
